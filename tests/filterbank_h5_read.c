...
int main(int argc, char * argv[])
{
  ...
  herr_t status = 0;
  while (
    (status = filterbank_h5_read(&fbh5)) >= 0
    && read_count <= 10
  ) {printf("data: %p\n", fbh5.data);
    size_t s = 0;
    for (int t=0; !failed && t < fbh5.ds_data.dimchunks[0]; t++) {
      for (int i = 0; !failed && i < fbh5.ds_data.dimchunks[1]; i ++) {
        for (int c = 0; !failed && c < fbh5.ds_data.dimchunks[2]; c ++) {
          int chan = ((read_count%chunks)*fbh5.ds_data.dimchunks[2]+c);
          float sample_exp = t*1000.0 + chan + 1.0;
          ...
          if (sample != sample_exp) {
            printf("Read-chunk %d, t%d:p%d:c%d = %f != %f (s=%ld)\n", read_count, t, i, c, sample, sample_exp, s);
            failed = 1;
          }
    ...
    for (int d = 0; d < fbh5.ds_data.rank; d ++) {
      printf("next hyperslab_start[%d] = %ld\n", d, fbh5.ds_data.hyperslab_start[d]);
    }
  }
  filterbank_h5_free(&fbh5);
  filterbank_h5_close(&fbh5);
  if (status != 1) {
    printf("Final read status not positive: %d.\n", status);
    failed = 1;
  }
  if (read_count != 4*chunks) {
    printf("Expected %d chunks, not %d.\n", 4*chunks, read_count);
    failed = 1;
  }
  return failed;
}